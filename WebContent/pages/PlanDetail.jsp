	...
	<div id="wrapper">
		<div id="plan">
			<div id="planHeader">
				...
				<div id="planDate">
					企画日：<%
					out.print(new SimpleDateFormat("yyyy年MM月dd日").format(plan.getPlanDatetime()));
				%>
				</div>
			</div>
			<div id="title">
				...
				<div id="startEnd">
					...
					<%
						if (plan.getStartDate() != null) {
																out.print(new SimpleDateFormat("yyyy年MM月dd日 HH時mm分").format(plan.getStartDate()));
															}
															if (plan.getEndDate() != null) {
																out.print("～");
																out.print(new SimpleDateFormat("yyyy年MM月dd日 HH時mm分").format(plan.getEndDate()));
															}
					%>
					<table id="evaluation">
						<tr>
							<td>
								<form action="/HomeSystem/fc/PlanDetail" method="post">
									<input type="submit" name="evaluationSubmit"
										...
										<%out.println(good);%>
									> <input type="hidden" name="evaluation" value="1"> <input
							...
							<%-- 										<%out.println(hold);%> --%>
							<!-- 									> <input type="hidden" name="evaluation" value="2"> <input -->
							<!-- 										type="hidden" name="planId" -->
							<%-- 										value=<%out.println(plan.getPlanId());%> --%>
							<!-- 									> -->
							<!-- 								</form> -->
							...
							<td><form action="/HomeSystem/fc/PlanDetail" method="post">
									<input type="submit" name="evaluationSubmit"
										...
										<%out.println(bad);%>
									> <input type="hidden" name="evaluation" value="3"> <input
										type="hidden" name="planId"
										value=<%out.println(plan.getPlanId());%>
									>
								</form></td>
		...
		<hr>
		<form action="/HomeSystem/fc/PlanDetail" enctype="multipart/form-data"
			method="post"
		>
			<input type="hidden" name="planId"
				value="<%out.print(plan.getPlanId());%>"
			...
			<input type="file" name="file" id="file1" style="display: none;"
				onchange="$('#fake_input_file_1').text($(this)[0].files[0].name)"
			> <input type="button" class="btn btn-2 btn-2c uploadBtn"
				value="ファイル選択" onClick="$('#file1').click();"
			> <span id="fake_input_file_1"></span> <br> <input type="file"
				name="file" id="file2" style="display: none;"
				onchange="$('#fake_input_file_2').text($(this)[0].files[0].name)"
			> <input type="button" class="btn btn-2 btn-2c uploadBtn"
				value="ファイル選択" onClick="$('#file2').click();"
			> <span id="fake_input_file_2"></span> <br> <input type="file"
				name="file" id="file3" style="display: none;"
				onchange="$('#fake_input_file_3').text($(this)[0].files[0].name)"
			> <input type="button" class="btn btn-2 btn-2c uploadBtn"
				value="ファイル選択" onClick="$('#file3').click();"
			> <span id="fake_input_file_3"></span> <br> <input
				type="submit" name="commentSubmit" value="送信"
				class="btn btn-2 btn-2c commentSubmit"
			>
		</form>
				...
				boolean isPlanner = false;
								for (CommentBean comment : commentList) {
									...
									if (comment.getDeleteFrag() == 0) {
										...
										for (FileBean file : commentFileList) {
										    if(file.getCommentNo() == comment.getCommentNo()){
					                                            ...
					                                            if (plan.getPlanner().equals(user.getUserId()) && plan.getExecuteFlag() == 0) {
					                                                ...
					                                                + "\"/><input type=\"hidden\" name=\"fileName\" value=\""
					                                                                ...
					                                                                + "\"/><input type=\"hidden\" name=\"commentNo\" value=\""
					                                                                + file.getCommentNo()
					                                                                + "\"/><input type=\"hidden\" name=\"commentFileDelete\" value=\"del\"/></form>");
					                                                out.println("</div>");
					                                            }
					                                            out.println("<div class=\"fileName\">" + file.getDataName()
					                                            + "</div>");
...
// 										out.println("<div class=\"commentFileUpload\">");
...
// 										out.println("</div>");//commenFileUpload
